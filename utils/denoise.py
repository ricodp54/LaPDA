...
def patch_based_denoise(model, pcl_noisy, patch_size=1000, seed_k=6, seed_k_alpha=1):
    ...
    while i < num_patches:
        ...
        try:
            patches_denoised_temp, patches_denoised_adapt_temp = model.denoise_langevin_dynamics(curr_patches)
...
def denoise_large_pointcloud(model, pcl, cluster_size=30000, seed=0):
    device = pcl.device
    pcl = pcl.cpu().numpy()
